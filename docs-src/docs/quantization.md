...
Contrast that with FP32, where the dynamic range is \(\pm 3.4\ x\ 10^{38}\), and the precision is approximately \(4.2\ x\ 10^9\) (note that the resolution in floating point formats the precision worsens as we get further from 0, but this makes no difference in the context of this discussion).  
...
To give a bit more flexibility to the integer formats, a **scale factor** is used to map the dynamic range of the tensor to the integer format range. But still we remain with the issue of having a significantly lower number of representable values, that is - much lower resolution.  
...
The result of multiplying two \(n\)-bit integers is, at most, a \(2n\)-bit number. In convolution layers, such multiplications are accumulated \(c\cdot k^2\) times, where \(c\) is the number of input channels and \(k\) is the kernel width. Hence, to avoid overflowing, the accumulator must be at least \(2n + c\cdot k^2\)-bits wide. In many cases 32-bit accumulators are used, however for INT4 and lower it might be possible to use less than 32 -bits, depending on the expected use cases and layer widths.
