...
import distiller
from distiller.modules import LSTM, LSTMCell
import torch
...
def test_basic():
    ...
    lstm = LSTM(3, 5, 4, False, False, 0.0, True)
...
def test_conversion():
    ...
    lc_man1 = LSTMCell.from_pytorch_impl(lc_pth)
    ...
    l_man1 = LSTM.from_pytorch_impl(l_pth)
...
def test_forward_lstm(input_size, hidden_size, num_layers, bidirectional):
    ...
    lstm_man = LSTM.from_pytorch_impl(lstm)
    lstm.eval()
    lstm_man.eval()
    ...
    lstm_man = LSTM(input_size, hidden_size, num_layers, bidirectional=bidirectional)
