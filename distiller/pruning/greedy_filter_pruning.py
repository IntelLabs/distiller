...
def greedy_pruner(pruned_model, app_args, fraction_to_prune, pruning_step, test_fn, train_fn):
    ...
    while total_macs > fraction_to_prune * dense_total_macs:
        ...
        save_checkpoint(pruned_model, compression_sched=scheduler, top1=prec1, arch=arch, dataset=dataset,
    ...
    scheduler = create_scheduler(pruned_model, zeros_mask_dict)
    save_checkpoint(pruned_model, arch=arch, dataset=dataset, compression_sched=scheduler, top1=prec1,
                    name='_'.join(("greedy", str(fraction_to_prune))), dir=msglogger.logdir)
